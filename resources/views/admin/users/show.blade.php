      ...
      <fieldset>
        ...
        <div class="form-group" data-sr="wait 0.6s, then enter bottom and hustle 100%">
            ...
            <div class="col-lg-10 input-grup">
              ...
              <input type="text" name="name" value="{{$user->name}}" class="form-control text-right" placeholder="اسم">
              @error('name')
          ...
          <div class="form-group" data-sr="wait 0.6s, then enter bottom and hustle 100%">
              ...
              <div class="col-lg-10 input-grup">
                  <i class="fa fa-user"></i>
                <input type="email" name="email" value="{{$user->email}}" class="form-control text-right" placeholder="البريد الاكتروني">
                @error('email')
                <span class="help-block text-danger">{{$message}}</span>
                @enderror
              </div>
              ...
              <div class="col-lg-10">
                <select class="form-control" multiple="" name="roles[]" data-title="أختر عده خيارات">
                  @foreach ($roles as $value)
                    <option @foreach ($user->roles as $role)
                       @if ($role->id==$value->id)
                         @selected(true)
                        @endif
