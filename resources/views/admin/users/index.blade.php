...
<table id="no-more-tables" class="table table-bordered" role="table">
    ...
    <thead>
        <tr>
            ...
            <th width="30%" class="text-right">البريد الاكتروني</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td data-title="التحكم" class="text-center">
                <a href="{{route('users.edit',$value->id)}}" class="btn btn-default btn-xs"><i class="fa fa-pencil-square"></i> تعديل</a>
                <a href="{{route('users.destroy',$value->id)}}" onclick="event.preventDefault();document.getElementById('form{{$value->id}}').submit()" class="btn btn-danger btn-xs"><i class="fa fa-plus-square"></i> حذف</a>
                <form action="{{route('users.destroy',$value->id)}}" id="form{{$value->id}}" method="post">
                    @csrf
                    @method("DELETE")
                </form>
            </td>
