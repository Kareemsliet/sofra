...
<table id="no-more-tables" class="table table-bordered" role="table">
    ...
    <thead>
        <tr>
            ...
            <th width="30%" class="text-right">الوصف</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td data-title="التحكم" class="text-center">
                <a href="{{route('payment-methods.edit',$value->id)}}" class="btn btn-default btn-xs"><i class="fa fa-pencil-square"></i> تعديل</a>
                <a href="{{route('payment-methods.destroy',$value->id)}}" onclick="event.preventDefault();document.getElementById('form{{$value->id}}').submit()" class="btn btn-danger btn-xs"><i class="fa fa-plus-square"></i> حذف</a>
                <form action="{{route('payment-methods.destroy',$value->id)}}" id="form{{$value->id}}" method="post">
                    @csrf
                    @method("DELETE")
                </form>
            </td>
...
<div class="col-xs-12 mt30 text-center">
    {{$payment_methods->onEachSide(4)->links()}}
</div>
@endsection
