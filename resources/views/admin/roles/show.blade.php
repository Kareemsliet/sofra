...
<div class="well bs-component" data-sr="wait 0s, then enter left and hustle 100%">
    ...
    <form class="form-horizontal" method="post" enctype="multipart/form-data" action="{{route('roles.update',$role->id)}}">
        @csrf
        @method("PUT")
      ...
      <fieldset>
        ...
        <div class="form-group" data-sr="wait 0.6s, then enter bottom and hustle 100%">
          ...
          <div class="col-lg-10 input-grup">
            ...
            <input type="text" name="name" value="{{$role->name}}" class="form-control text-right" placeholder="اسم ">
            @error('name')
        ...
        <div class="form-group" data-sr="wait 0.3s, then enter bottom and hustle 100%">
            <label for="select" class="col-lg-2 control-label">الصلاحيات</label>
            <div class="col-lg-10">
              <select class="form-control" multiple="" name="permissions[]" data-title="أختر عده خيارات">
                @foreach ($permissions as $value)
                <option @foreach ($role->permissions as $per)
                    @if ($per->id==$value->id)
                      @selected(true)
                    @endif
                 @endforeach  value="{{$value->id}}">{{$value->name}}</option>
                @endforeach
            </select>
            @error('permissions')
            <span class="help-block text-danger">{{$message}}</span>
