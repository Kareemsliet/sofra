...
<div class="well bs-component" data-sr="wait 0s, then enter left and hustle 100%">
    ...
    <form class="form-horizontal" method="post" enctype="multipart/form-data" action="{{route('regions.update',$region->id)}}">
        @csrf
        @method("PUT")
      ...
      <fieldset>
        ...
        <div class="form-group" data-sr="wait 0.6s, then enter bottom and hustle 100%">
          <label for="inputUser"  class="col-lg-2 control-label">أسم الحي</label>
          <div class="col-lg-10 input-grup">
              <i class="fa fa-user"></i>
            <input type="text" name="name" value="{{$region->name}}" class="form-control text-right" placeholder="اسم الحي">
            @error('name')
        ...
        <div class="form-group" data-sr="wait 0.3s, then enter bottom and hustle 100%">
            <label for="select" class="col-lg-2 control-label">أختر المدينة</label>
            <div class="col-lg-10">
              <select class="form-control" name="city_id" >
                @foreach ($cities as $value)
                <option @if ($region->city_id==$value->id)
                     @selected(true)
