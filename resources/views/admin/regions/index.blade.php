...
@section('head')
@php
$title="الاحياء/الشوارع"
@endphp
@endsection
@section('content')
<ul class="breadcrumb pb30">
    <li><a href="#">الرئيسية</a></li>
    <li class="active">الاحياء/الشوارع</li>
</ul>
...
<table id="no-more-tables" class="table table-bordered" role="table">
    ...
    <thead>
        <tr>
            ...
            <th width="30%" class="text-right">المدينة</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td data-title="المدينة">{{$value->city->name}}</td>
            <td data-title="التحكم" class="text-center">
                <a href="{{route('regions.edit',$value->id)}}" class="btn btn-default btn-xs"><i class="fa fa-pencil-square"></i> تعديل</a>
                <a href="{{route('regions.destroy',$value->id)}}" onclick="event.preventDefault();document.getElementById('form{{$value->id}}').submit()" class="btn btn-danger btn-xs"><i class="fa fa-plus-square"></i> حذف</a>
                <form action="{{route('regions.destroy',$value->id)}}" id="form{{$value->id}}" method="post">
                    @csrf
                    @method("DELETE")
                </form>
            </td>
